...
function showEventNames()
{
    ...
    $stmt = $conn->query("SELECT name FROM events");
    if ($stmt->rowCount() > 0) {
...
function showEventsInfo()
{
    $db = new Database();
    $conn = $db->getConnection();
    $stmt = $conn->query("SELECT name, location, type FROM events");
    if ($stmt->rowCount() > 0) {
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo '<div class="event-details">';
            echo '<p>PATROCINADO POR FINDURMUSIC <span>' . '</span></p>';
            echo '<p>NOMBRE: <span>' . $row["name"] . '</span></p>';
            echo '<p>TIPO: <span>' . $row["type"] . '</span></p>';
            echo '<p>LOCALIZACIÓN: <span>' . $row["location"] . '</span></p>';
            echo '</div>';
        }
    } else {
        echo '<p style="color: red;">No hay eventos creados.</p>';
...
function getEventDetails($eventName)
{
    ...
    $stmt = $conn->prepare("SELECT name, location, type FROM events WHERE name = :eventName");
